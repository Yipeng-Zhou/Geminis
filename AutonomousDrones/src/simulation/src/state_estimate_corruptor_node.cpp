...
class StateEstimateCorruptorNode {
 public:
	StateEstimateCorruptorNode() {
		// Subscribers
		pose_sub     = nh.subscribe("/Quadrotor_1/unity_ros/Quadrotor/Sensors/IMU/pose", 1, &StateEstimateCorruptorNode::OnPose, this);
		velocity_sub = nh.subscribe("/Quadrotor_1/unity_ros/Quadrotor/Sensors/IMU/twist", 1, &StateEstimateCorruptorNode::OnVelocity, this);

		// Publishers
		corrupted_pose_pub     = nh.advertise<geometry_msgs::PoseStamped>("/Quadrotor_1/pose_est", 1);
		corrupted_velocity_pub = nh.advertise<geometry_msgs::TwistStamped>("/Quadrotor_1/twist_est", 1);
		corrupted_state_pub    = nh.advertise<nav_msgs::Odometry>("/Quadrotor_1/current_state_est", 1);
 ...
 private:
	...
	void PublishCorruptedState(geometry_msgs::TwistStamped const& corrupt_twist) {
		...
		corrupted_state.child_frame_id = "Quadrotor_1";
	...
	void PublishBodyTransform(geometry_msgs::PoseStamped const& pose) {
		...
		transformStamped.child_frame_id = "Quadrotor_1";
		...
		transformStamped_base.child_frame_id = "Quadrotor_1_base";
