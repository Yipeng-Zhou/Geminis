...
class StateEstimateCorruptorNode {
 public:
	StateEstimateCorruptorNode() {
		// Subscribers
		pose_sub     = nh.subscribe("/Quadrotor_2/unity_ros/Quadrotor2/Sensors/IMU/pose", 1, &StateEstimateCorruptorNode::OnPose, this);
		velocity_sub = nh.subscribe("/Quadrotor_2/unity_ros/Quadrotor2/Sensors/IMU/twist", 1, &StateEstimateCorruptorNode::OnVelocity, this);

		// Publishers
		corrupted_pose_pub     = nh.advertise<geometry_msgs::PoseStamped>("/Quadrotor_2/pose_est", 1);
		corrupted_velocity_pub = nh.advertise<geometry_msgs::TwistStamped>("/Quadrotor_2/twist_est", 1);
		corrupted_state_pub    = nh.advertise<nav_msgs::Odometry>("/Quadrotor_2/current_state_est", 1);
 ...
 private:
	...
	void PublishCorruptedState(geometry_msgs::TwistStamped const& corrupt_twist) {
		...
		corrupted_state.child_frame_id = "Quadrotor_2";
	...
	void PublishBodyTransform(geometry_msgs::PoseStamped const& pose) {
		...
		transformStamped_base.header.stamp = pose.header.stamp;
		transformStamped_base.header.frame_id = "world";
		transformStamped_base.child_frame_id = "Quadrotor_2_base";
		transformStamped_base.transform.translation.x = pose.pose.position.x;
		transformStamped_base.transform.translation.y = pose.pose.position.y;
		transformStamped_base.transform.translation.z = 0;
		transformStamped_base.transform.rotation.x = pose.pose.orientation.x;
		transformStamped_base.transform.rotation.y = pose.pose.orientation.y;
		transformStamped_base.transform.rotation.z = pose.pose.orientation.z;
		transformStamped_base.transform.rotation.w = pose.pose.orientation.w;
		br_base.sendTransform(transformStamped_base);
	}
...
int main(int argc, char* argv[]) {
	...
	ros::init(argc, argv, "StateEstimateCorruptorNode_Quadrotor_2");
